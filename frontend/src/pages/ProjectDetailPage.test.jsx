...
describe("ProjectDetailPage area catalog", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("keeps the detail page catalog-only and does not request dashboard data", async () => {
    resolvePage();
  ...
  it("shows loading while the project and area catalog are being fetched", async () => {
  ...
  it("shows the empty state and preserves the catalog creation destination", async () => {
    resolvePage([]);
    ...
    fireEvent.click(within(empty).getByRole("button", { name: "Thêm phân khu" }));
    expect(await screen.findByTestId("location")).toHaveTextContent("/catalog?project=project-a");
  });
