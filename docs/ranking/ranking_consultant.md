...
| Read hierarchical score + coverage + evidence links | *(none)* | **PROPOSED — new**: `GET /ranking/hierarchical?external_project_id=` | Extends the existing `GET /ranking` response shape [`src/api/ranking.py:122-135`] with `hierarchical_score`, per-grain (`M`/`P`/`A`/`U`) coverage, and a `feature_justification_id` per contributing value for evidence drill-down |
...
| **W6** | W2–W5, whichever shipped | Frontend `GET /ranking/hierarchical` consumption, per-grain coverage display, `score_mode`/comparability-warning UI (T18), monitoring (§24.11's observability, extended with per-grain-`quality_status` counts and the D37/D38 metrics) | No new grains — surfaces what W2–W5 already compute, with `score_mode` and the comparability warning made visible for the first time | A sales user can see `hierarchical_score`, its `score_mode`, its band, and drill into the evidence behind each contributing grain; a comparison view spanning unequal-coverage units shows the warning (T18) | Frontend reverts to `ranking_scores.score` only; no backend change needed | Depends on which of W2–W5 actually shipped |
